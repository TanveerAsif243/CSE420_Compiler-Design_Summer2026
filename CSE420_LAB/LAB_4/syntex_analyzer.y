...
statement : variable_decl
	  ...
	  | FOR LPAREN expression_statement expression_statement expression RPAREN statement
	  {
			...
			ExprStmtNode* initStmt = dynamic_cast<ExprStmtNode*>($3->get_ast_node());
			ExprStmtNode* condStmt = dynamic_cast<ExprStmtNode*>($4->get_ast_node());
			ExprNode* initExpr = initStmt ? initStmt->release_expr() : nullptr;
			ExprNode* condExpr = condStmt ? condStmt->release_expr() : nullptr;
			if(initStmt) { delete initStmt; $3->set_ast_node(nullptr); }
			if(condStmt) { delete condStmt; $4->set_ast_node(nullptr); }
