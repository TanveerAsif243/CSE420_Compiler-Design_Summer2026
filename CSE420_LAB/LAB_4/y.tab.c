...
static const yytype_uint8 yyprhs[] =
{
      ...
      62,    64,    66,    68,    72,    79,    81,    86,    88,    90,
      93,    95,    98,   100,   102,   104,   106,   114,   120,   128,
     134,   140,   144,   146,   149,   151,   156,   158,   162,   164,
     168,   170,   174,   176,   180,   182,   186,   189,   192,   194,
     196,   201,   205,   207,   209,   212,   215,   217,   218,   222
};
...
static const yytype_int8 yyrhs[] =
{
      ...
      10,    -1,    11,    -1,    13,    -1,     5,    -1,    51,    34,
      52,    -1,    51,    34,    52,    32,    36,    33,    -1,    52,
      -1,    52,    32,    36,    33,    -1,    38,    -1,    54,    -1,
      53,    54,    -1,     1,    -1,    53,     1,    -1,    49,    -1,
      44,    -1,    55,    -1,    47,    -1,     6,    28,    55,    55,
      57,    29,    54,    -1,     3,    28,    57,    29,    54,    -1,
       3,    28,    57,    29,    54,     4,    54,    -1,     7,    28,
      57,    29,    54,    -1,    19,    28,    52,    29,    35,    -1,
      14,    57,    35,    -1,    35,    -1,    57,    35,    -1,    52,
      -1,    52,    32,    57,    33,    -1,    58,    -1,    56,    25,
      58,    -1,    59,    -1,    59,    26,    59,    -1,    60,    -1,
      60,    24,    60,    -1,    61,    -1,    60,    20,    61,    -1,
      62,    -1,    61,    21,    62,    -1,    20,    62,    -1,    27,
      62,    -1,    63,    -1,    56,    -1,    52,    28,    64,    29,
      -1,    28,    57,    29,    -1,    36,    -1,    37,    -1,    56,
      22,    -1,    56,    23,    -1,    65,    -1,    -1,    65,    34,
      58,    -1,    58,    -1
};
...
static const yytype_uint16 yyrline[] =
{
     ...
     520,   526,   533,   541,   549,   557,   565,   589,   603,   618,
     632,   653,   666,   677,   690,   735,   777,   786,   826,   835,
     870,   879,   914,   924,   964,   974,  1049,  1074,  1099,  1112,
...
{
  "$end", "error", "$undefined", "IF", "ELSE", "CHAR", "FOR", "WHILE",
  "DO", "BREAK", "INT", "FLOAT", "DOUBLE", "VOID", "RETURN", "SWITCH",
  ...
  "unit", "func_definition", "enter_func", "param_list",
  "compound_statement", "enter_scope_variables", "variable_decl",
...
#line 566 "24241370.y"
    {
			...
			// expression_statement wraps the first two for-clauses in ExprStmtNode.
			// Transfer the underlying expressions to ForNode so ownership and types stay correct.
			ExprStmtNode* initStmt = dynamic_cast<ExprStmtNode*>((yyvsp[(3) - (7)])->get_ast_node());
			ExprStmtNode* condStmt = dynamic_cast<ExprStmtNode*>((yyvsp[(4) - (7)])->get_ast_node());
			ExprNode* initExpr = initStmt ? initStmt->release_expr() : nullptr;
			ExprNode* condExpr = condStmt ? condStmt->release_expr() : nullptr;
			if(initStmt) { delete initStmt; (yyvsp[(3) - (7)])->set_ast_node(nullptr); }
			if(condStmt) { delete condStmt; (yyvsp[(4) - (7)])->set_ast_node(nullptr); }
...
#line 633 "24241370.y"
    {
			...
			// Preserve printf(ID); in the AST so TAC contains the call.
...
#line 1229 "24241370.y"
    {
		...
		// Represent postfix increment with one owned operand (prevents double deletion).
		UnaryOpNode* incNode = new UnaryOpNode(
			"post++",
			(ExprNode*)(yyvsp[(1) - (2)])->get_ast_node(),
			(yyvsp[(1) - (2)])->getvartype()
		);
		(yyval)->set_ast_node(incNode);
	}
    break;

  case 65:
/* Line 1792 of yacc.c  */
#line 1245 "24241370.y"
    {
		...
		// Represent postfix decrement with one owned operand (prevents double deletion).
		UnaryOpNode* decNode = new UnaryOpNode(
			"post--",
			(ExprNode*)(yyvsp[(1) - (2)])->get_ast_node(),
			(yyvsp[(1) - (2)])->getvartype()
		);
		(yyval)->set_ast_node(decNode);
	}
    break;

  case 66:
/* Line 1792 of yacc.c  */
#line 1263 "24241370.y"
    {
...
//	if(ast_root) {
//		outlog << endl << "Constructed AST:" << endl;
//		outlog << "----------------------------------------" << endl;
//		ast_root->print_ast(outlog, 0);
//		outlog << "----------------------------------------" << endl;
//	}
